-- Create the mcp_servers table if it doesn't exist yet
...
DROP POLICY IF EXISTS "Approved servers are publicly viewable" ON public.mcp_servers;
CREATE POLICY "Approved servers are publicly viewable"
  ON public.mcp_servers FOR SELECT
  USING (approved = true);

DROP POLICY IF EXISTS "Authenticated users can submit servers" ON public.mcp_servers;
...
CREATE INDEX IF NOT EXISTS idx_mcp_servers_approved ON public.mcp_servers(approved);

-- Grant select permission to anon/authenticated roles
...
-- Stored Procedure to safely increment install counts from the client
...
GRANT EXECUTE ON FUNCTION increment_install_count(UUID) TO anon, authenticated;

-- Seed the 8 real MCP servers (ignoring duplicates if they already exist)
INSERT INTO public.mcp_servers (name, slug, description, category, github_url, install_command, config_snippet, author_name, author_email, install_count, star_rating, approved) 
VALUES
